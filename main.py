...
# ── 하이디라오(훠궈) 서울 매장 위치 ─────────────────────────────
# 서울에서 영업 중인 하이디라오 지점들의 대략적인 좌표입니다.
# (지점은 변동될 수 있으니, 정확한 주소·좌표는 확인 후 수정하세요.)
HAIDILAO_STORES = [
    {"지점": "명동점",   "지역": "서울 중구 명동",           "lat": 37.5638, "lon": 126.9847},
    {"지점": "강남점",   "지역": "서울 강남구 강남역 인근",   "lat": 37.5017, "lon": 127.0263},
    {"지점": "홍대점",   "지역": "서울 마포구 홍대 인근",     "lat": 37.5555, "lon": 126.9235},
    {"지점": "건대점",   "지역": "서울 광진구 건대입구 인근", "lat": 37.5404, "lon": 127.0700},
    {"지점": "영등포점", "지역": "서울 영등포구 영등포 인근", "lat": 37.5171, "lon": 126.9032},
    {"지점": "대학로점", "지역": "서울 종로구 대학로 인근",   "lat": 37.5822, "lon": 127.0018},
]
...
# ── 하이디라오(훠궈) 서울 매장 ──────────────────────────────────
st.subheader("🍲 하이디라오 서울 매장")
st.caption("※ 서울에서 영업 중인 지점의 대략적인 위치입니다. 지점·주소는 변동될 수 있어 방문 전 확인을 권장합니다.")

# 위에서 정해 둔 매장 좌표 목록을 표(데이터프레임)로 바꿉니다.
haidilao_df = pd.DataFrame(HAIDILAO_STORES)

# 영화관 지도와 구분되도록 하이디라오 매장은 빨간 점으로 찍습니다.
st.map(haidilao_df, size=60, color="#e23744")

# 매장 이름과 지역을 표로 보여 줍니다. (좌표 열은 표에서는 숨김)
st.dataframe(
    haidilao_df[["지점", "지역"]],
